...
export default function MovieDetail() {
  ...
  useEffect(async () => {
    var c = await axios.get(
      `https://api.themoviedb.org/3/movie/${id}?api_key=fed69657ba4cc6e1078d2a6a95f51c8c`
    );

    var getPersonCast = await axios.get(
      `https://api.themoviedb.org/3/movie/${c.data.id}/credits?api_key=fed69657ba4cc6e1078d2a6a95f51c8c`
    );



    console.log("ccc", c.data.id);
    // setGenre([...c.data.genres, genre]);
    console.log("getPersonCast", getPersonCast.data);

    setPersonCast(getPersonCast.data.cast.slice(0, 4));
    setMovie(c.data);
  }, []);

  function goToGenre(genr) {
    console.log("genr", genr);
    history.push(`/genre/${genr.id}/${genr.name}`);
  }
